...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    //ts->irq=gpio_to_irq(5);       //If not defined in client
    if (ts->irq)
    {
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    printk("Sanny Debug:begin to probe goodix ts\n");//Sanny add for debug
//    printk("Sanny Debug\n");//Sanny add for debug
...
s32 gtp_i2c_read_no_rst(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    msgs[0].len   = GTP_ADDR_LENGTH;
    msgs[0].buf   = &buf[0];
    //rockchip 才有scl_rate设置，对于树莓派需要去掉该配置  
    //msgs[0].scl_rate = 300 * 1000;    // for Rockchip, etc.
...
static int goodix_ts_init(void)
{
    ...
    printk("i2c_add_driver,result is 0x%08x", ret);
    return ret; 
}
